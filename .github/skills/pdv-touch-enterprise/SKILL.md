...
## Atualizacao Fiscal e Banco (2026-08-11)

### Banco oficial do projeto
- Banco de desenvolvimento: `pdv_touch_dev`.
...
### Estado fiscal NFC-e
- A tela **Configurações Fiscais NFC-e** e a fonte principal dos dados fiscais do estabelecimento.
- O painel **Pdv_Sefaz** deve consumir essa configuracao e exibir ambiente, pendencias, fila, prontidao e bloqueio de producao.
- Certificado A1 pode ser importado/referenciado e armazenado com seguranca pelo Electron `safeStorage`.
- A senha do certificado deve ser usada somente no momento da validacao/preparacao e depois limpa da UI.
- Existe fluxo para **Preparar XML homologação**:
  - bloqueia producao;
  - usa certificado A1 seguro;
  - monta XML NFC-e de homologacao;
  - gera chave de acesso e QR Code preparatorio com CSC/CSC ID;
  - assina localmente com `node-forge`;
  - valida estrutura minima do XML assinado antes de qualquer envio.

### O que ainda nao deve ser tratado como pronto para producao
- Envio SOAP real para SEFAZ-SP ainda nao esta liberado para venda real.
- Validacao XSD oficial, XMLDSig/C14N homologada, QR Code/cHashQRCode final e tratamento completo de `cStat` ainda precisam ser validados em homologacao.
- Producao NFC-e deve continuar bloqueada ate homologacao oficial e validacao do contador.

### Arquivos fiscais-chave
- `fiscal.md`
- `electron/main.cjs`
...
### Checklist fiscal antes de concluir alteracoes
1. Confirmar que producao continua bloqueada.
2. Confirmar que senha do certificado nao fica persistida.
3. Rodar `node -c electron/main.cjs` quando alterar Electron.
4. Rodar `npm run build`.
5. Atualizar `fiscal.md`, `README.md`, `Readme_bd.md` e esta skill quando mudar contrato fiscal ou banco.
